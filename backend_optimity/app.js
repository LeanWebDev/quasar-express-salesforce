...
const conn = new jsforce.Connection({ loginUrl: process.env.SF_URL });
conn.login(
...
app.get("/account/all", function(req, res) {
  let records = [];
  let q =
    "SELECT Id, Name, Account_Number__c, Industry, Type FROM Account LIMIT 20";
...
app.get("/contact/all", function(req, res) {
  let records = [];
  let q = "SELECT Id, Name, Department FROM Contact LIMIT 20";
  // Connection login setup
  conn.login(
    process.env.SF_USERNAME,
    process.env.SF_PASSWORD + process.env.SF_SECURITY_TOKEN,
...
app.get("/case/all", function(req, res) {
  let records = [];
  let q =
    "SELECT Id, Subject, Status, Type, CaseNumber, Description, OwnerId, Reason, Origin, CreatedDate, ClosedDate FROM Case ORDER BY CreatedDate DESC LIMIT 20";
  // Connection login setup
  conn.login(
    process.env.SF_USERNAME,
    process.env.SF_PASSWORD + process.env.SF_SECURITY_TOKEN,
...
app.get("/email-message/all", function(req, res) {
  let records = [];
  let q =
    "SELECT ParentId, Status, Subject, TextBody, HtmlBody, MessageDate, CreatedById FROM EmailMessage ORDER BY MessageDate DESC LIMIT 10";
  // Connection login setup
  conn.login(
    process.env.SF_USERNAME,
    process.env.SF_PASSWORD + process.env.SF_SECURITY_TOKEN,
...
// Subscriptions -> related (to this account)

app.get("/subscription/related/:accountId", function(req, res) {
  let records = [];
  let accountId = req.params.accountId;
  console.log(`THIS IS THE ACCOUNT ID TO QUERY WITH!!!! ${accountId}`);
  ...
  conn.login(
    ...
    function(loginErr, loginResult) {
      ...
      console.log("Related Subscriptions for this account -> all(:accountId");
